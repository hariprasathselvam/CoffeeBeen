...
const PaymentCardDetials = ({route}) => {
  return (
    <View style={{flex:1,backgroundColor:COLOURS.LightGreen}}>
      ...
      <View style={{marginTop:"35%",backgroundColor:COLOURS.Primary,flex:1,borderTopStartRadius:50,borderTopEndRadius:50}}>
        <View style={{height:200,width:300,backgroundColor:COLOURS.Blue,alignSelf:"center",borderRadius:20,bottom:"15%"}}>
            <Text style={{alignSelf:"flex-end",paddingHorizontal:30,top:"10%"}}>{route.params.date}</Text>
            <Image source={Images.chip} style={{height:40,width:40,left:"10%",top:"15%"}}/>
            <Text style={{fontSize:fontSizes.FONT18,fontWeight:"500",left:"10%",top:"25%",letterSpacing:2}}>{route.params.cardnumber}</Text>
                <Text style={{fontSize:fontSizes.FONT18,fontWeight:"500",left:"10%",top:"30%",letterSpacing:2}}>{route.params.name}</Text>
        </View>
      </View>
    </View>
  )
